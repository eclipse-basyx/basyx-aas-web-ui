...
export function checkSemanticIdEclassIrdi (keyValue: string, semanticId: string): boolean {
  ...
  if (!keyValue.startsWith('0173-1#') && !keyValue.startsWith('0173/1///')) {
    return false
  }
  ...
  if (keyValue.startsWith('0173-1#')) {
    ...
    return (
      getEquivalentEclassSemanticIds(keyValue).some(equivalentSemanticId => {
...
export function checkSemanticIdIecCdd (keyValue: string, semanticId: string): boolean {
  ...
  if (!semanticId.startsWith('0112/')) {
    return false
  }
  if (!keyValue.startsWith('0112/')) {
    return false
  }
...
export function checkSemanticIdIri (keyValue: string, semanticId: string): boolean {
  ...
  if (!semanticId.startsWith('http://') && !semanticId.startsWith('https://')) {
    return false
  }
  if (!keyValue.startsWith('http://') && !keyValue.startsWith('https://')) {
    return false
  }

  if (keyValue.endsWith('/')) {
    keyValue = keyValue.slice(0, Math.max(0, keyValue.length - 1))
  }
  ...
  if (new RegExp(/\/\d{1,}\/\d{1,}\/{0,1}$/).test(semanticId) || new RegExp(/\/\d{1,}\/\d{1,}\//).test(semanticId)) {
    ...
    return (
      getEquivalentIriSemanticIds(keyValue).some(equivalentSemanticId => {
...
export function getEquivalentIriSemanticIds (semanticId: string): any[] {
  ...
  // e.g. IRI
  if (semanticId.endsWith('/')) {
    semanticIds.push(semanticId.slice(0, Math.max(0, semanticId.length - 1)))
  } else {
