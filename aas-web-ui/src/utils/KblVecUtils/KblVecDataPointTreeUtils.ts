...
export function buildDataPointTree (root: Element): DataPointTreeNode[] {
  ...
  for (const [tagName, elements] of groupedChildren.entries()) {
    ...
    for (const [index, element] of elements.entries()) {
      const result = buildDataPointTreeNode(element, tagName, 1, ++sequence, index + 1)
...
function buildDataPointTreeNode (
  ...
  const displayLabel = elementId === '' ? (siblingIndex > 1 ? `${tagName} [${siblingIndex}]` : tagName) : `${tagName} (${elementId})`
  const exportIdShort = elementId === '' ? toSubmodelElementIdShort(`${tagName}_${siblingIndex}`) : toSubmodelElementIdShort(elementId)
...
export function resolveExportPropertyLabel (
  ...
  let relativeSegments = selectedNodeIndex === -1 ? pathSegments.slice(-2) : pathSegments.slice(selectedNodeIndex + 1)

  if (relativeSegments.length === 0) {
    relativeSegments = [pathSegments.at(-1) ?? 'Value']
  }
  ...
  const rawLabel = relativeSegments.map(segment => sanitizeSegment(segment)).join('_') || 'Value'
