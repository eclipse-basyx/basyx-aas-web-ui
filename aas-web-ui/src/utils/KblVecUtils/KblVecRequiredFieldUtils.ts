...
export function inferRequiredFieldValues (
  ...
  const hasDataPoints = points.length > 0
  ...
  for (const definition of requiredFieldDefinitions) {
    ...
    const fromElements = findElementTextByAliases(doc, definition.key, definition.aliases)
  ...
  paths.shortId = ''
  values.assetKind = hasDataPoints ? '' : 'Instance'
  paths.assetKind = ''
  ...
  for (const point of points) {
    ...
    for (const alias of normalizedAliases) {
      if (tokens.includes(alias)) {
        score += 6
        hasAliasMatch = true
      } else if (labelNormalized.includes(alias)) {
        score += 3
        hasAliasMatch = true
      }
    }
...
function findElementTextByAliases (
  ...
  const maxScan = allElements.length
  ...
  for (let index = 0; index < maxScan; index++) {
    ...
    let hasAliasMatch = false
    for (const alias of normalizedAliases) {
      if (normalizedName === alias || pathTokens.includes(alias)) {
        score += 5
        hasAliasMatch = true
      } else if (normalizeForLookup(path).includes(alias)) {
        score += 2
        hasAliasMatch = true
      }
    }
