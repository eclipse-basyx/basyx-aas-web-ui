...
export function normalizeAssetKind (input: string): aasTypes.AssetKind {
  ...
  if (normalized === 'type') {
    return aasTypes.AssetKind.Type
  }
  if (normalized === 'role') {
    return aasTypes.AssetKind.Role
  }
  if (normalized === 'notapplicable' || normalized === 'not applicable') {
    return aasTypes.AssetKind.NotApplicable
  }
