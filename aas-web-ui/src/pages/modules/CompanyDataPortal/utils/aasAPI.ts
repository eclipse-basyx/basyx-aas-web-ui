...
export async function createAAS (
  ...
  shell.displayName = aasDisplayName && Array.isArray(aasDisplayName) && aasDisplayName.length > 0
    ? aasDisplayName.map(
        dn => new aas.types.LangStringNameType(dn.language ?? 'en', dn.text ?? 'Company Data AAS'),
      )
    : [new aas.types.LangStringNameType('en', 'Company Data AAS')]
...
export async function uploadConsolidatedDataFile ({
  ...
  const res = await fetch(url, { method: 'PUT', body: form })
  if (!res.ok) {
    throw new Error(`ConsolidatedDataFile upload failed: ${res.status} ${await res.text()}`)
  }
}
