...
export function createSharedSchemas (messages: AbacValidationMessages) {
  ...
  const aclEntrySchema = z
    .looseObject({
      USEATTRIBUTES: z
        .string()
        .min(1, { error: messages.aclAttributesRequired })
        .optional(),
      ATTRIBUTES: z
      ...
      RIGHTS: z
        ...
        .min(1, { error: messages.aclRightsRequired }),
      ACCESS: z.enum(['ALLOW', 'DISABLED'], {
        error: messages.aclAccessMustBeAllowOrDisabled,
      }),
    })
    .refine(
  ...
  const objectEntrySchema = z.union([
    z.strictObject({
      ROUTE: z
        .string()
        .min(1, { error: messages.objectEntryValueRequired }),
    }),
    z.strictObject({
      IDENTIFIABLE: z
        .string()
