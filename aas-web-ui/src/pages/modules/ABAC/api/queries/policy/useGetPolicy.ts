...
export function useGetPolicy (versionId: Ref<string | undefined> | string | undefined) {
  ...
  return useQuery({
    queryKey: computed(() => [ABAC_CACHE_KEYS.POLICY, apiUrl.value, idRef.value]),
    enabled: computed(() => idRef.value !== undefined && !!apiUrl.value?.trim()),
    queryFn: async () => {
      const response = await client.getPolicyVersion(idRef.value!)
      if (!response.success || !response.data) {
        throw new Error('Failed to load ABAC policy version')
      }
