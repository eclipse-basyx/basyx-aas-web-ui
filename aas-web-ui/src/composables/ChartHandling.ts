  ...
  function prepareLegend (yVariables: any) {
    return {
      formatter: (seriesName: any, opts: any) => {
        ...
        // check if the yVariable exists
        if (yVariables.length > index // check if the yVariable has an unit (embeddedDataSpecification) -> take the first one (TODO: make this more generic in the future)
          && yVariables[index]) {
          unit = '[' + unitSuffix(yVariables[index]) + ']'
        }
