...
import { safeSegment } from '@/../shared/utils/StringUtils'
...
function cloneWithoutRuntimeFields (value: unknown): unknown {
  ...
  if (record) {
    ...
    for (const [key, item] of Object.entries(record)) {
      if (runtimeKeys.has(key) && !shouldPreservePath(record, key) && !shouldPreserveId(record, key)) {
        continue
      }
...
export function isCrossOriginHttpUrl (url: string, referenceUrl: string): boolean {
  ...
  if (!referenceOrigin) {
    return absoluteTargetOrigin !== null
  }
...
function collectFileBindings (raw: unknown, clean: unknown, bindings: FileBinding[]): void {
  if (!raw || !clean || typeof raw !== 'object' || typeof clean !== 'object') {
    return
  }
...
export function useAASXPackaging (): {
  ...
  async function buildSupplementaryParts (
    ...
    for (const [index, fileBinding] of fileBindings.entries()) {
