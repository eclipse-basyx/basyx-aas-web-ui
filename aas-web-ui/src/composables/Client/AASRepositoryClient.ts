import type { types as aasTypes, jsonization } from '@aas-core-works/aas-core3.1-typescript'
...
export function useAASRepositoryClient () {
  ...
  async function aasIsAvailableById (aasId: string): Promise<boolean> {
    ...
    if (aasEndpoint && aasEndpoint.trim() !== '') {
      return await aasIsAvailable(aasEndpoint.trim())
    }
  ...
  async function fetchAssetInformationById (aasId: string): Promise<any> {
    ...
    if (aasEndpoint && aasEndpoint.trim() !== '') {
      fetchAssetInformation(aasEndpoint.trim())
    }
  ...
  async function getSubmodelRefsById (aasId: string): Promise<Array<any>> {
    ...
    if (aasEndpoint && aasEndpoint.trim() !== '') {
      return getSubmodelRefs(aasEndpoint.trim())
    }
  ...
  async function downloadAasx (aas: any): Promise<void> {
    if (!aas || Object.keys(aas).length === 0 || !aas.id) {
      return
    }
    ...
    if (Array.isArray(submodelRefList) && submodelRefList.length > 0) {
      ...
      if (aasRepoUrl.split('/').length - 1 > 2) {
        aasSerializationPath = aasRepoUrl.slice(0, Math.max(0, aasRepoUrl.lastIndexOf('/'))) // strips everything after the last slash (http://localhost:8081/shells -> http://localhost:8081)
      }
