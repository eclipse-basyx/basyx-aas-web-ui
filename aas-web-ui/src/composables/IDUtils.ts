...
export function useIDUtils () {
  ...
  function generateIri (type: string): string {
    ...
    // Check type
    if (!type || !['Asset', 'AssetAdministrationShell', 'Submodel'].includes(type)) {
      type = ''
    }
    ...
    // Check idPrefix
    let idPrefix = idPrefixFromStore !== '' && regex.test(idPrefixFromStore) ? idPrefixFromStore : defaultIdPrefix
