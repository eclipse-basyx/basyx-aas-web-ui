    ...
    <v-card>
      <v-card-title class="d-flex align-center">
        ...
        <v-text-field
          ...
          @update:model-value="searchInJson"
        />
        <!-- Line wrap toggle -->
        <v-btn
          v-if="jsonContent"
      ...
      <v-card-text>
        ...
        <div v-else class="json-container" :class="{ 'word-wrap-enabled': wordWrapEnabled }">
          ...
          >
                        ...
                        <code ref="codeElement" class="language-json" v-html="formattedJson || jsonContent" />
                    </pre>
        </div>
      </v-card-text>
    </v-card>
...
<script lang="ts" setup>
  ...
  function processSelectedNode (): void {
    ...
    try {
      ...
      console.error('Error processing selected node:', error_)
      error.value = `Error processing JSON data: ${error_ instanceof Error ? error_.message : 'Unknown error'}`
    } finally {
      loading.value = false
    }
  }
  ...
  async function copyToClipboard (): Promise<void> {
    if (jsonContent.value) {
      try {
        ...
        console.error('Failed to copy JSON to clipboard', error_)
      }
    }
  }
