...
<script lang="ts" setup>
  ...
  onMounted(async () => {
    try {
      ...
      if (!ifcContainer.value) {
        ifcContainer.value = document.querySelector('#ifcContainer') as HTMLDivElement
      }
  ...
  function setupRenderer (): void {
    ...
    world.renderer.three.setClearColor(0x22_22_22, 1)
  ...
  function setupViewCube (): void {
    ...
    try {
      ...
      ifcContainer.value.append(viewCube)
  ...
  function setupHighlighter (): void {
    ...
    try {
      ...
      highlighter.events.select.onHighlight.add((modelIdMap: any) => {
        ...
        selectedElementInfo.value = modelIds.length > 0 ? `Element ID: ${modelIds[0]}` : null
  ...
  function setupPropertiesTable (): void {
    ...
    if (!propertiesTableContainer.value) {
      propertiesTableContainer.value = document.querySelector('#properties-container') as HTMLDivElement
    }
    ...
    try {
      ...
      if (propertiesTable) {
        try {
          ...
          console.warn('Failed to clean up previous properties table:', error)
        }
      ...
      if (propertiesTable) {
        ...
        propertiesTableContainer.value.append(propertiesTable)
  ...
  function filterProperties (): void {
    if (propertiesTable && propertySearchQuery.value !== undefined) {
      propertiesTable.queryString = propertySearchQuery.value === '' ? null : propertySearchQuery.value
    }
  }
