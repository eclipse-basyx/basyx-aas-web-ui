    ...
    <v-card class="d-flex flex-column" color="transparent">
      ...
      <v-card-text class="d-flex flex-column fill-height" @click.self="closeDialog">
        ...
        <div v-else-if="error" class="d-flex align-center justify-center fill-height" @click.self="closeDialog">
          <v-sheet class="mx-auto pa-6" elevation="8" max-width="500" rounded="lg">
            <v-empty-state
              color="error"
        ...
        <div v-else class="d-flex align-center justify-center fill-height" @click.self="closeDialog">
          ...
          <!-- JSON Preview -->
          <v-sheet v-else-if="fileContentType === 'json'" class="preview-code" elevation="4" rounded="lg">
            <JSONPreview :submodel-element-data="submodelElementData" />
          </v-sheet>

          <!-- XML Preview -->
          <v-sheet v-else-if="fileContentType === 'xml'" class="preview-code" elevation="4" rounded="lg">
            <XMLPreview :submodel-element-data="submodelElementData" />
          </v-sheet>
...
<script setup lang="ts">
  ...
  async function loadFile (): Promise<void> {
    ...
    try {
      ...
      console.error('Error loading file:', error_)
      error.value = true
