    ...
    <v-card class="mb-4">
      ...
      <v-card-text class="pt-1">
        ...
        <v-select
          ...
          return-object
          variant="outlined"
          @update:model-value="emitSegment"
        />
        <!-- Record Selection -->
        <v-row>
          <v-col cols="12" md="6">
...
<script lang="ts" setup>
  ...
  const segmentType = computed(() => {
    ...
    const semanticIds = new Set(selectedSegment.value.semanticId.keys.map((semanticId: any) => semanticId.value))

    // check if the semanticIds contain the semanticId for InternalSegment
    if (semanticIds.has('https://admin-shell.io/idta/TimeSeries/Segments/InternalSegment/1/1')) {
      return 'InternalSegment'
    }

    // check if the semanticIds contain the semanticId for LinkedSegment
    if (semanticIds.has('https://admin-shell.io/idta/TimeSeries/Segments/LinkedSegment/1/1')) {
      return 'LinkedSegment'
    }

    // check if the semanticIds contain the semanticId for ExternalSegment
    if (semanticIds.has('https://admin-shell.io/idta/TimeSeries/Segments/ExternalSegment/1/1')) {
  ...
  function initializeTimeSeriesData (): void {
    ...
    const promises = recordsArray.map((record: any) => {
