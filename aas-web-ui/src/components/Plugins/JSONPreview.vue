      ...
      <v-card-text>
        ...
        <div v-else class="json-container" :class="{ 'word-wrap-enabled': wordWrapEnabled }">
          ...
          <pre
            ...
            @scroll="syncScroll"
          >
                        <code ref="codeElement" class="language-json" v-html="formattedJson" />
                    </pre>
        </div>
      </v-card-text>
    </v-card>
...
<script lang="ts" setup>
  ...
  async function initialize (): Promise<void> {
    ...
    try {
      ...
      console.error('Error loading JSON file:', error_)
      error.value = `Error loading JSON file: ${error_ instanceof Error ? error_.message : 'Unknown error'}`
    } finally {
      loading.value = false
    }
  }
