  ...
  <v-container fluid class="pa-0">
    ...
    <v-expansion-panels>
      ...
      >
        ...
        <v-expansion-panel-text>
          <!-- Dynamically display each property's metadata and endpoint -->
          <v-card
            v-for="(property, propertyIndex) in collection.properties"
    ...
    <v-dialog v-model="deleteDialog" max-width="500px" persistent>
      <v-card>
        ...
        <v-card-text
          >Are you sure you want to delete the selected properties?</v-card-text
        >
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" @click="closeDeleteDialog">
            Cancel
          </v-btn>
...
export default defineComponent({
  ...
  methods: {
    fetchData() {
      ...
      // Find all SubmodelElementCollection
      const submodelElementCollections =
        this.submodelElementData.submodelElements.filter(
          (element: any) => element.modelType === "SubmodelElementCollection"
        );

      // Fetch details for each collection
      submodelElementCollections.forEach((collection: any) => {
        const propertiesCollection = this.findNestedElement(
          collection.value,
          "properties"
        );
        if (propertiesCollection) {
          ...
          const properties = propertyDefinitions.map(
            (propertyDefinition: any) => {
              return {
                ...
                title: this.getPropertyValue(propertyDefinition, "title"),
                unit: this.getPropertyValue(propertyDefinition, "unit"),
                endpoint: `${baseEndpoint}${this.getPropertyValue(
                  propertyDefinition,
                  "href"
                )}`,
                semanticId: propertyDefinition.semanticId?.value || "",
    ...
    updateSelectedProperties() {
      ...
      this.isAnyPropertySelected; // Trigger reactivity
    },
    openDeleteDialog() {
