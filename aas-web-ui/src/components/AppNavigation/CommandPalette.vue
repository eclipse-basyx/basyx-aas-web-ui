<template>
  <v-dialog v-model="dialogModel" absolute contained :max-width="500">
    <v-sheet border rounded="lg">
      ...
      <v-card-text class="pa-0" style="max-height: 400px; overflow-y: auto">
        ...
        <v-list v-if="search?.trim() && filteredRoutes.length > 0" density="compact" nav slim>
          ...
          >
            <template #prepend>
              <v-icon class="me-2" icon="mdi-arrow-right" size="small" />
            </template>
          </v-list-item>
        </v-list>

        <!-- Divider between routes and shortcuts -->
        <v-divider v-if="search?.trim() && filteredRoutes.length > 0" />
...
<script setup lang="ts">
  ...
  const commands = computed(() => {
    ...
    for (const category of Object.keys(grouped)) {
      result.push({ id: `header-${category}`, category, isHeader: true }, ...grouped[category])
    }
